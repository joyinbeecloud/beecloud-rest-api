...
1. 一项计划由每期费用、周期、试用天数等参数构成。一项计划可以附属多条订阅。
...
4. “试用天数(`trial_days`)”用于调整该项计划的周期性扣费时间，默认值为0。“试用天数(`trial_days`)”与订阅的属性“试用结束时间(`trial_end`)”二者配合，来决定当前**订阅**的扣费时间。**注意：**“试用结束时间(`trial_end`)”如果设置数值，会覆盖“试用天数(`trial_days`)”值。
5. 某项**计划**的周期性扣费时间点的示例（**假设`trial_end = null`，按月计费**）：
	- 无试用期，先付费后使用：`trial_days`设为`0`会从订阅成功的当天开始扣费，以后按月扣费。
	- 无试用期，先使用后付费：`trial_days`设为当月天数（假设为30），从订阅成功的第31天起按月扣费。
	- 有试用期3天，先付费后使用：`trial_days`设为`3`，从订阅成功的第4天开始扣费，以后按月扣费。
	- 有试用期3天，先使用后付费：`trial_days`需要设置为 *3 + 当月的天数(假设30)*，从订阅成功的第34天扣费，以后按月扣费。

**关于订阅的说明：**

1. 订阅是依附于计划的，一条订阅记录能且只能依附于一项计划。
2. 商户需要提供购买者的`buyer_id`以及银行卡等认证信息（见下）。
3. 对于类似收取电费的场景，计划的收费金额`fee`应当是电费的单价，用户每月使用的度数在订阅中的`amount`设置，在每次扣款时间点之前，商户的系统需要更新每个注册用户对应订阅的`amount`数值。
4. 商户根据需要设定“试用结束时间(`trial_end`)”，默认值为`null`。当前订阅会从`trial_end`的下一天进行第一次扣费，并在之后按照计划中设定的时间间隔，周期性扣费。如果该参量设定了数值，会覆盖掉所依附的计划中的“试用天数(`trial_days`)”的值。该参量也是用来调整周期性扣费时间的。
5. 如果商户需要从用户订阅的当天就开始扣费，需要设定计划的`trial_end = null`且订阅的`trial_days = 0`。
...
3. 订阅参数`trial_end`用于设置试用的截止时间，如果设置了会忽略订阅计划中的试用天数`trial_days`，可以用来统一设置订阅用户的收费时间点  
