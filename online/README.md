...
total_fee | Integer | 订单总金额 | 必须是正整数，单位为分，范围在大于等于1分小于等于20亿分，如果渠道限额小于20亿分的以渠道为准 | 1 | 是
...
return_url | String | 同步返回页面| 支付渠道处理完请求后,当前页面自动跳转到商户网站里指定页面的http路径，**<mark>中间请勿有#,?等字符</mark>** | http://beecloud.cn/returnUrl.jsp | 当channel参数为 ALI\_WEB 或 ALI\_QRCODE 或 UN\_WEB或JD\_WAP或JD\_WEB或BD\_WEB时为必填
...
refund_no | String | 商户退款单号 | 格式为:退款日期(8位) + 流水号(3~24 位)。请自行确保在商户系统中唯一，且退款日期必须是发起退款的当天日期,同一退款单号不可重复提交，否则会造成退款单重复。流水号可以接受数字或字母，建议使用数字，但不可接受“000” 。百度要求退款单号长度小于22位| 201506101035040000001 | 是
