...
analysis | Map | 分析数据 | 用于统计分析的数据，将会在控制台的统计分析报表中展示，**<mark>用户自愿上传</mark>** | 包括以下基本字段：`os_version(系统版本)` `model(手机型号)` `app_version(应用版本号)` `device_id(设备ID)` `category(类别)` `browser_type(浏览器类型)` `browser_version(浏览器版本)` | 否
