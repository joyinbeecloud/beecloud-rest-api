...
1. 接受SDK请求参数
2. 校验app\_id以及app\_sign(使用test_secret)
3. 校验请求参数合法性
4. 将订单存入 sandbox订单表中
5. 返回Mock的请求结果
6. 提供伪造的异步通知API
7. 异步通知API修改sanbox订单状态并发送sandbox webhook （参数列表与真实webhook相同）
...
##### ii. 进入API端
4. 校验app\_id, timestamp, app\_sign 
5. 校验参数是否合法
6. 返回特定url（BeeCloud提供的伪造支付页面）
...
##### iii. 返回SDK端
7. 检查返回结果
8. 弹出伪造支付宝界面，需包含支付及取消两个按钮
9. 如点击支付按钮需显示支付成功，并调用伪造的异步通知接口
...
app_sign | String | 加密签名 | 算法: md5(app\_id+timestamp+test\_secret)，32位16进制格式,不区分大小写 | b927899dda6f9a04afc57f21ddf69d69 | 是
...
### 3.4 sandbox 异步通知(指定ID)

#### URL:   */1/rest/sandbox/{app\_id}/{id}*
#### Method: *GET*
#### id : 支付订单唯一标识
