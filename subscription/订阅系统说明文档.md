...
4. “免费试用天数(`trial_days`)”用于调整该项计划的首次扣费时间，默认值为0。如果设置了数值，则表示某用户从注册订阅到开始被收费的间隔天数为`trial_days`。见如下示例中的计划周期性扣费的时间点（**按月计费，不考虑订阅的参数`trial_end`**）：
	- 无试用期，先付费后使用：`trial_days`设为`0`会从订阅成功的下一天开始扣费，以后按月扣费。BeeCloud计费规则会按照自然月扣费。
...
2. 当前的订阅*允许同一个用户多次订阅同一个计划*，当多次发起订阅请求的时候请务必确认这不是错误操作，因为每一次成功的订阅都会收取用户的费用。 可以通过计划的参数`anti_duplicate`来设定。 
3. 对于类似收取电费的场景，计划的收费金额`fee`应当是电费的单价，用户每月使用的度数在订阅中的`amount`设置，在每次扣款时间点之前，商户的系统需要更新每个注册用户对应订阅的`amount`数值。
4. 商户根据需要设定“试用截止时间点(`trial_end`)”，该参数应为Unix毫秒数，默认值为`null`。若`trial_end`值为`null`，开始收费时间请参阅计划中`trial_days`的说明。如果此参数设置了时间点，计划中的“试用天数(`trial_days`)”将被忽略，当前订阅直接从`trial_end`的下一天进行第一次扣费，之后按照计划中设定的时间间隔，周期性扣费。该参量可以用来统一订阅用户的收费时间。
5. 用户第一次注册成功的情况下，`webhook`会返回`card_id`，该id由{`bank_name`、`card_no`、`id_name`、`id_no`、`mobile`}共同决定，可以直接用于发起订阅。
6. 订阅取消时，商户后端可以根据扣费需求设定参数`at_period_end`。此参数用于确定是否应在下次扣款之后取消当前订阅。
	- 先付费后使用：调用取消订阅接口时，不传`at_period_end`参数或者设定为`false`。
	- 先使用后付费：调用取消订阅接口时，传入`at_period_end`参数且设定为`true`。商户也可以不使用此参数，此时取消订阅立刻生效。取决于商户的用户政策。
