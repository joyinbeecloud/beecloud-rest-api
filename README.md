...
bill_no | String | 商户订单号 | 32个字符内，数字和/或字母组合，请自行确保在商户系统中唯一 | 201506101035040000001 | 是
...
refund_no | String | 商户退款单号 | 格式为:退款日期(8位) + 流水号(3~24 位)。请自行确保在商户系统中唯一，且退款日期必须是发起退款的当天日期。流水号可以接受数字或英文字符，建议使用数字，但不可接受“000” | 201506101035040000001 | 是
bill_no | String | 商户订单号 | 32个字符内，数字和/或字母组合，请自行确保在商户系统中唯一 | 201506101035040000001 | 是 
...
channel| String | 渠道类型 | 根据不同场景选择不同的支付方式 | WX、WX\_NATIVE、WX\_JSAPI、ALI、ALI\_APP、ALI\_WEB、ALI\_QRCODE、UN、UN\_APP、UN\_WEB(详见附注）| 是
bill_no | String | 商户订单号 | 32个字符内，数字和/或字母组合，请自行确保在商户系统中唯一 | 201506101035040000001 | 否
...
refund_no | String | 商户退款单号 | 格式为: 退款日期(8位) + 流水号(3~24 位)。请自行保证在商户系统中唯一，且退款日期必须是退款发起的当天日期。流水号可以接受数字或英文字符，建议使用数字，但不可接受“000”。 | 201506101035040000001 | 是
